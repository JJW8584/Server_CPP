...
int main()
{
	...
	for (int32 i = 0; i < 3; i++)
	{
		...
		int32 gold = 100;
		WCHAR name[100] = L"사용자1";
		TIMESTAMP_STRUCT ts = { 2025, 7, 30 };
	...
	// Read
	{
		...
		*/
		...
		while (dbConn->Fetch())
		{
			std::wcout << "Id : " << outId << " Gold : " << outGold << " name : " << outName << endl;
			std::wcout << "Date : " << outDate.year << '/' << outDate.month << '/' << outDate.day << endl;
		}
